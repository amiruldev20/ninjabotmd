...
cmd: (string | RegExp)[],
tag: string[],
callback: (mel: Proto, property: ICommandHandler.CommandProperty) => Promise<any> | any,
...
public emit = (mel: Proto): void => {
const ev = this.getCommand(mel.string);
try {
if (!('event' in ev)) return;
const access = this.getAccess(mel, ev);
if (access === 200)
ev.event.callback(mel, {
...ev,
});
} catch (e) {
...
public getAccess = (mel: Proto, event: ICommandHandler.CommandProperty): void | Promise<proto.WebMessageInfo> | 200 => {
let CONFIG!: [string | string[] | boolean, string];
 
if (event.event?.query && event.query.length === 0) CONFIG = [event.event.query!, 'query'];
if (event.event?.group && !mel.validator.isGroup) CONFIG = [event.event.group!, 'group'];
if (event.event?.owner && !mel.validator.isOwner) CONFIG = [event.event.owner!, 'owner'];
if (
event.event?.mediaCustomReply &&
!(event.event.mediaCustomReply instanceof Array
? event.event.mediaCustomReply.some((a) => a in client.messageType && client.messageType[a] === (mel.quotedMsg?.type[1] ?? mel.type[1]))
: event.event.mediaCustomReply in client.messageType &&
  client.messageType[event.event.mediaCustomReply] === (mel.quotedMsg?.type[1] ?? mel.type[1]))
)
CONFIG = [event.event.mediaCustomReply, 'mediaCustomReply'];
 
if (typeof CONFIG === 'object' && CONFIG.length > 0) {
return this.action(mel, CONFIG[0], CONFIG[1], {
prefix: event.prefix,
cmd: event.cmd,
});
}
 
if (event.event?.wait) this.action(mel, event.event.wait as boolean | string, 'wait');
return 200;
};
